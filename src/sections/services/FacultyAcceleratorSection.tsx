...
interface FacultyAcceleratorSectionProps {
  heading?: string;
  meta?: string;
  paragraphs?: string[];
}
...
export default function FacultyAcceleratorSection({
  heading = 'Institutional Integration & Behavior Change Accelerator',
  meta = '4-6 Months',
  paragraphs,
}: FacultyAcceleratorSectionProps) {
  const contentParagraphs = paragraphs && paragraphs.length > 0 ? paragraphs : defaultParagraphs;
      ...
      <div className="page-margin max-content relative z-10">
        <ScrollReveal delay={0.1}>
          <div className="group bg-gradient-to-br from-black to-magenta border border-white/10 rounded-2xl p-6 md:p-8 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-pink/20">
            ...
            {meta && (
              <span className="font-display text-sm font-medium text-pink mb-4 block">{meta}</span>
            )}
            {contentParagraphs.map((p, i) => (
              <p
                key={i}
                className={`font-body text-sm leading-relaxed text-white/70 ${i < contentParagraphs.length - 1 ? 'mb-4' : 'last:mb-0'}`}
              >
                {p}
              </p>
            ))}
